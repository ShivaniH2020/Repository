...
public class HillCipher_2
{
	...
	{
		keytomatrix(key, len);
		int d = determinant(keymatrix, len); d = d % 26;
		if (d == 0)
		{
	...
	public int determinant(int A[][], int N)
	{
		...
		else
		{
			...
			{
				int m[][] = new int[N - 1][N - 1]; for (int i = 1; i < N; i++)
				{
					int j2 = 0;
					for (int j = 0; j < N; j++)
					{
	...
	void trans(int fac[][], int r) {
		...
		System.out.println("\nInverse key:"); matrixtoinvkey(inv, r);
	}
	public int mi(int d)
	{
	...
	{
		HillCipher_2 obj = new HillCipher_2(); 
		...
		if (sq != (long) sq)
			System.out.println("Invalid key length!!! Does notform a square matrix..."); 
		else
		{
			int s = (int) sq;
			if (obj.check(key, s)) {
				System.out.println("Result:"); obj.divide(line, s); obj.cofact(obj.keymatrix, s);
			}
		}
	}
}
